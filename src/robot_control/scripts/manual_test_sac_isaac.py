...
class GazeboEnv(Node):
    ...
    def __init__(self):
        ...
        self.next_state = np.zeros(21) # 0~19:lidara, 20:distance to the goal
    ...
    def step(self, action, time_steps, max_episode_steps):
        ...
        if(clash_sum > 0):
           ...
           self.done = True    
           reward = 0        
        elif( 22.0 < body_pose[0] < 26.0 and 
              -26.0 < body_pose[1] < -22.0):
            ...
            reward = -0.2
        elif(self.next_state[20] < self.prev_dist):
            reward = 1
        else:
            reward = 0

        self.prev_dist = self.next_state[20]
